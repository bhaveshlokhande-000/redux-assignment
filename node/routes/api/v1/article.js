...
const { userAuth, authRole } = require("../../../middleware/auth");
...
//UPDATE ARTICLE
router.put("/:id", userAuth, authRole(), async (req, res) => {
...
//DELETE ALL ARTICLE
router.get("/deleteAll", userAuth, authRole(), async (req, res) => {
  const result = await getAllArticles();
...
//DELETE ARTICLE
router.delete("/:id", userAuth, authRole(), async (req, res) => {
  const result = await getArticleById(req.params.id);
  if (!result.response.success) {
    return res.status(result.status).json({
      error: [{ msg: result.response.error[0].msg }],
